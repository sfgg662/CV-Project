...
class LaplacianConv(nn.Module):
    def __init__(self):
        super().__init__()
        self.lap_conv = nn.Conv2d(3, 3, 3, padding=1, groups=3, bias=False)
        lap_kernel = torch.tensor([[0, -1, 0], [-1, 4, -1], [0, -1, 0]], dtype=torch.float32)
        weight = lap_kernel.repeat(3, 1, 1, 1)  # [3, 1, 3, 3]
        self.lap_conv.weight.data = weight

    def forward(self, img, pool_p):
        pooled = F.avg_pool2d(img, kernel_size=pool_p, stride = pool_p)
        return self.lap_conv(pooled)
...
p1 = 4
with torch.no_grad():
    content_laplacian1 = lap_calculator.forward(content_img, pool_p = p1)

p2 = 16
with torch.no_grad():
    content_laplacian2 = lap_calculator.forward(content_img, pool_p = p2)
...
while run[0] <= 300: 
    ...
    def closure():
        ...
        syn_laplacian1 = lap_calculator(syn_img, pool_p = p1)
        syn_laplacian2 = lap_calculator(syn_img, pool_p = p2)
        lap_loss = lap_weight1 * F.mse_loss(content_laplacian1, syn_laplacian1) + lap_weight2 * F.mse_loss(content_laplacian2, syn_laplacian2)

        total_loss = content_weight * content_loss + style_weight * style_loss + lap_loss
        total_loss.backward()
